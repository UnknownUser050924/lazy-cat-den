const EFFECTS: Record<string, { label: string; count: number }> = {
  ...
  fireflies: { label: "萤火晚灯", count: 16 },
  petals: { label: "花信缓落", count: 13 },
  comets: { label: "流星来信", count: 12 },
  aurora: { label: "极光轻纱", count: 9 },
  embers: { label: "壁炉余温", count: 15 },
  moonorbit: { label: "月亮轨道", count: 12 },
};
...
export function applyProfileEffect(container: HTMLElement | null, requested: string) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    node.style.setProperty("--dur", `${(5.8 + (i % 5) * 1.1).toFixed(1)}s`);
    node.style.setProperty("--sz", `${4 + (i % 4) * 2}px`);
